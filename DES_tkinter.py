...
def decrypt_button_clicked():
    ...
    if len(ciphertext) == 1 and len(key) == 1:
        ...
        if ciphertext_ascii > 255 or key_ascii > 1023:
            ...
            ciphertext_binary = format(
                ciphertext_ascii, '08b')  # 将密文转化为8bit二进制list
...
# 创建输入框和标签
plaintext_label = tk.Label(window, text="明文（8位）：",
                           font=("微软雅黑", 12), bg="#F9F9F9")
plaintext_entry = tk.Entry(window, font=("微软雅黑", 12), width=20, bd=2)

ciphertext_label = tk.Label(window, text="密文（8位）：",
                            font=("微软雅黑", 12), bg="#F9F9F9")
